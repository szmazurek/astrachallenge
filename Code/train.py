...
from dataloader import load_data, load_data_no_split
...
def train_model(
    ...
    # (Initialize logging)
    experiment = wandb.init(project='U-Net-AZ', resume='allow', entity="mazurek")
...
def train_model_kfold(
    ...
    # Load data
    train_set_full = load_data_no_split()
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(train_set_full)):
        ...
        for tag, value in model.named_parameters():
                ...
                if not (torch.isinf(value.grad) | torch.isnan(value.grad)).any():
                        histograms['Gradients/' + tag] = wandb.Histogram(value.grad.data.cpu()) """
        # Did not change the variable names due to convenience
